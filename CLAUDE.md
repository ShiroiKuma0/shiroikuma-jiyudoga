...
| In-app update check (both platforms) | `App.vue` calls `getUpdateInfo()` — the android fork's checker, repointed at **our** repo (`REPO_ID = ShiroiKuma0/shiroikuma-jiyudoga`), so the banner offers the releases `/publish-version` cuts. Two rules keep it honest, both learned from false banners: (1) **the upstream-base pins are never compared** — `versionNumberGt` strips them from both sides and orders builds by FORK_VERSION then the `+NNN` counter alone. Pins are identifiers, not ordering material: their shape changed (dot-joined → `+`-grouped, date → date + `HH-MM`) and the 2026-08-12 switch to UTC moved a pin date BACKWARDS for the same commit, which is exactly how the stale `+006` release outranked the running `+010`. The stripper accepts both shapes plus the degraded `+g<sha>`, since old tags stay on GitHub forever. (2) **`[0]` of the releases API is not the newest** — despite the documented reverse-chronological order, on 2026-08-12 this repo's list returned `+006` ahead of `+009` and `+008`, so we fetch `per_page=10`, drop drafts/prereleases and pick the max with our own comparator | `helpers/android/system.js`, `helpers/android/utils.js`, `_scripts/fork-version.js` |
